...
public class Home_fragment extends Fragment{
    ...
    private FloatingActionButton floatingActionButtonNotification;
    ...
    // Declare a boolean to track the notification state
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        floatingActionButtonNotification = view.findViewById(R.id.focusNotification);
...
//                if (!userStatus.equals(currentStatus)) {
//                    currentStatus = userStatus;
//                    updateDeviceIcon(currentStatus);
//                }
        ...
        floatingActionButtonNotification.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                // Toggle the state
                if (isNotificationActive) {
                    // Change to inactive icon
                    floatingActionButtonNotification.setImageResource(R.drawable.ic_notification_off); // Change to your inactive icon
                    writeNotification(0);
                } else {
                    // Change back to active icon
                    floatingActionButtonNotification.setImageResource(R.drawable.ic_notification_active); // Change back to active icon
                    writeNotification(1);
                }
                // Update the state
                isNotificationActive = !isNotificationActive; // Toggle the boolean state
            }
        });
    ...
    private void writeNotification(int message) {
        ...
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference notificationRef = database.getReference(username + "/feature/Notification");

        // Write the message (0 or 1) to the specified location
        notificationRef.setValue(message).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Toast.makeText(getContext(), "Notification updated successfully.", Toast.LENGTH_SHORT).show();
            } else {
